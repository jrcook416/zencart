...
- **Plugin lifecycle:** IemsCountyAgency v1.9.5 is the supported forward version. Its v1.9.4 compatibility payload and v1.9.5 payload both retain the v1.9.2 shipping/address/payment schema, behavior, module files, and installer semantics. Upgrade only backfills missing delivery-rate configuration for an already installed delivery module, even when disabled; it does not uninstall modules or remove configuration. Uninstall still removes custom shipping/payment module configuration through native module removal while retaining every IEMS reference field and row, including category and mileage. Reinstall does not rerun category inference on retained schema.
- **Recovery versions:** v1.9.4 restores v1.9.2-equivalent source for databases already registered at v1.9.4, allowing a Git deployment to restore all files before Plugin Manager is used. Upgrade to v1.9.5 afterward. Neither version contains an Admin Orders display customization.
- **Initial checkout presentation (v1.9.1):** Before standard checkout has confirmed an Ordering Unit selection, the shipping page shows the normal delivery address, required selector, validation messages, and continue controls only. It suppresses the stock no-shipping warning and order-comments section until selection confirmation. Confirmed checkout restores shipping choices or the genuine no-method warning and comments. One-Page Checkout and later checkout pages retain their existing behavior.
- **Unit delivery address composition (v1.9.2):** The order delivery address retains the customer's company, first name, and last name, resolving each from the selected checkout delivery address, billing address, then default customer address. Its suburb is the canonical selected unit label, and its street, city, postcode, Indiana state, and United States country come from the validated managed unit destination. Pickup retains its independently configured recipient/company.
...
- Back up the database and deploy the complete source tree. Sites already registered at v1.9.4 regain their module/runtime files immediately from the compatibility payload; then upgrade to v1.9.5 through Plugin Manager. Valid historical county codes need no rewrite: `49` and `049` both migrate as Marion. Resolve any invalid county relationship/code reported by preflight before retrying. Confirm exact category/mileage definitions and review all migrated categories.
...
- Run all six deterministic harnesses for v1.9.4 and v1.9.5, all v1.9.2 regressions, and PHP lint for both recovery payloads. Their source-parity guards require the two payment modules, two shipping modules, absence of the withdrawn Orders observer/registration, and functional-source equality with v1.9.2 outside version metadata/tests. Installer coverage includes fresh foundation-schema installation, compatible county-code widths and invalid-code preflight, upgrades/repeated upgrades, retained category choices and mileage endpoints, malformed schema, installed/disabled/uninstalled delivery, missing-only rate backfill, and uninstall/reinstall.
