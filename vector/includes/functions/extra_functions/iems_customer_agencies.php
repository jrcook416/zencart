<?php
/**
 * functions_iems_customer_agencies
 *
 ...
 * @return bool
 */
function iems_customer_belongs_to_agency($customer_id, $agency_id, $ignore_cache = false)
{
    global $db;
    $sql = "SELECT customer_id
            FROM " . TABLE_CUSTOMERS_TO_AGENCY . "
            WHERE customer_id = " . (int)$customer_id . "
            AND agency_id = " . (int)$agency_id;
    if (!$ignore_cache) {
 ...
 * @return array
 */
function iems_agencies_customer_belongs_to($customer_id)
{
    global $db;
    $groups = [];

    $sql = "SELECT masterAgencyID, masterAgency, masterAgencyDescription
            FROM " . TABLE_CUSTOMER_AGENCIES . " cg
            LEFT JOIN " . TABLE_CUSTOMERS_TO_AGENCY . " ctg ON cg.masterAgencyID = ctg.agency_id
            WHERE customer_id = " . (int)$customer_id . "
            ORDER BY masterAgencyID, masterAgency";

    $results = $db->Execute($sql);

    foreach ($results as $result) {
        $agencies[$result['masterAgencyID']] = $result['masterAgencyDescription'];
//        $agencies[] = ['id' => $result['masterAgencyID'], 'text' => $result['masterAgencyDescription']];
    }
 ...
 * @return bool
 */
function iems_sync_customer_agency_assignments($customer_id, $groups)
{
 ...
 * @return bool
 */
function iems_assign_customer_to_agency($customer_id, $group_id)
{
 ...
 * @return bool
 */
function iems_remove_customer_from_agency($customer_id, $group_id)
{
    global $db;
 ...
 * @return int
 */
function iems_count_customers_in_agency($group_id)
{
    global $db;
 ...
 * @return string
 */
function iems_get_customer_agency_name($group_id)
{
    global $db;
    if (empty($group_id)) {
 ...
 */
function iems_cfg_select_customer_agency($group_id, $key = '', $name = '', $include_zero = true, $multiple = false)
{
    global $db;
    if (empty($name)) {
 ...
 * @return array
 */
function iems_get_all_customer_agencies()
{
    global $db;
 ...
 * @return int|string
 */
function iems_create_customer_agency($group_name, $group_comment)
{
    global $db;
    if (empty($group_name)) {
 ...
 * @return bool|string
 */
function iems_update_customer_agency($group_id, $data)
{
    global $db;
 ...
 * @return bool|string
 */
function iems_delete_customer_agency($group_id, $also_unassign_customers = true)
{
    global $db;
