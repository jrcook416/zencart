...
$assert($manifest['pluginVersion'] === 'v1.9.5', 'Manifest should identify plugin version v1.9.5.');
...
$behaviorRoot = dirname($pluginRoot) . '/v1.9.2';
$behaviorFiles = array_values(array_filter(
    $relativeFiles($behaviorRoot),
    static fn(string $path): bool => $path !== 'manifest.php' && !str_starts_with($path, 'tests/')
));
$currentFiles = $relativeFiles($pluginRoot);
$currentBehaviorFiles = array_values(array_filter(
    $currentFiles,
    static fn(string $path): bool => $path !== 'manifest.php' && !str_starts_with($path, 'tests/')
));
$assert(
    $currentBehaviorFiles === $behaviorFiles,
    'v1.9.5 functional source must contain exactly the v1.9.2 payload.'
);

$requiredModuleFiles = [
    'catalog/includes/modules/payment/iemsinvoice.php',
    'catalog/includes/modules/payment/iemsunit.php',
    'catalog/includes/modules/shipping/iemsdelivery.php',
    'catalog/includes/modules/shipping/iemspickup.php',
];
foreach ($requiredModuleFiles as $relativePath) {
    $assert(is_file($pluginRoot . '/' . $relativePath), 'Missing required recovery module: ' . $relativePath);
}
$assert(
    !is_file($pluginRoot . '/admin/includes/classes/observers/class.iems_orders_observer.php'),
    'The withdrawn Admin Orders observer must not be present.'
);
$adminAutoLoader = file_get_contents(
    $pluginRoot . '/admin/includes/auto_loaders/config.iems_county_agency_customers.php'
);
$assert(
    $adminAutoLoader !== false && !str_contains($adminAutoLoader, 'IemsOrders'),
    'The admin autoloader must not register the withdrawn Orders observer.'
);

foreach ($behaviorFiles as $relativePath) {
    $oldFile = $behaviorRoot . '/' . $relativePath;
    $newFile = $pluginRoot . '/' . $relativePath;
    $assert(
        hash_file('sha256', $oldFile) === hash_file('sha256', $newFile),
        'v1.9.5 behavior differs unexpectedly from v1.9.2: ' . $relativePath
    );
}
