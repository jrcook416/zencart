 ...
 * @version $Id: DrByte 2020 Nov 17 Modified in v1.5.7b $ modified by MAUARI for EXTRAFIELD and EMAIL CONFIRM
 * 07/09/2022
 */
...
// start extrafield
    if ((EXTRAFIELD_REQUIRED == 'true') && (!zen_not_null($extrafield)) ) {
        $error = true;
        $messageStack->add('create_account', ENTRY_EXTRAFIELD_ERROR);
    }	
    if ((EXTRAFIELD_REQUIRED2 == 'true') && (!zen_not_null($extrafield2)) ) {
        $error = true;
        $messageStack->add('create_account', ENTRY_EXTRAFIELD2_ERROR);
    }	
    if ((EXTRAFIELD_REQUIRED3 == 'true') && (!zen_not_null($extrafield3)) ) {
        $error = true;
        $messageStack->add('create_account', ENTRY_EXTRAFIELD3_ERROR);
    }	
    if ((EXTRAFIELD_REQUIRED4 == 'true') && (!zen_not_null($extrafield4)) ) {
        $error = true;
