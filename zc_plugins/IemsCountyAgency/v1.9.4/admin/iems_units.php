...
function iems_units_get_unit(int $unitId): ?array
{
    ...
    $sql =
        "SELECT unit_ID, county_ID, agency_ID, unit_identifier, unit_name,
                delivery_street_address, delivery_city, delivery_postcode, one_way_miles, status
...
function iems_units_add_input_errors(array $errorCodes): void
{
    ...
    $messages = [
        ...
        'address_incomplete' => ERROR_ADDRESS_INCOMPLETE,
        'street_length' => ERROR_STREET_LENGTH,
        'city_length' => ERROR_CITY_LENGTH,
        'postcode_length' => ERROR_POSTCODE_LENGTH,
        'street_format' => ERROR_ADDRESS_FORMAT,
        'city_format' => ERROR_ADDRESS_FORMAT,
        'postcode_format' => ERROR_ADDRESS_FORMAT,
        'one_way_miles_invalid' => ERROR_ONE_WAY_MILES_INVALID,
    ];
    foreach (array_unique($errorCodes) as $errorCode) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!in_array($postAction, ['save', 'deactivate', 'reactivate'], true)) {
        ...
        if ($agency !== null && IemsAgencyInput::countyNumber($agency['county_number'] ?? null) === null) {
            $input['errors'][] = 'agency_county_invalid';
            $messageStack->add(ERROR_AGENCY_COUNTY_INVALID, 'error');
        }
        ...
        if ($input['errors'] === []) {
            ...
            $streetAddress = $formValues['delivery_street_address'];
            $city = $formValues['delivery_city'];
            $postcode = $formValues['delivery_postcode'];
            $oneWayMiles = $formValues['one_way_miles'];
            $addressReady = $streetAddress !== '';
            if ($isCreate) {
                $sql =
                    "INSERT INTO " . TABLE_IEMS_UNITS . "
                        (county_ID, agency_ID, unit_identifier, unit_name,
                         delivery_street_address, delivery_city, delivery_postcode, one_way_miles,
                         status, date_added, last_modified)
                     VALUES
                        (:countyId, :agencyId, :identifier, :unitName,
                         NULLIF(:streetAddress, ''), NULLIF(:city, ''), NULLIF(:postcode, ''), NULLIF(:oneWayMiles, ''),
                         1, NOW(), NULL)";
                $sql = $db->bindVars($sql, ':countyId', $countyId, 'integer');
                $sql = $db->bindVars($sql, ':agencyId', $agencyId, 'integer');
                $sql = $db->bindVars($sql, ':identifier', $identifier, 'string');
                $sql = $db->bindVars($sql, ':unitName', $name, 'string');
                $sql = $db->bindVars($sql, ':streetAddress', $streetAddress, 'string');
                $sql = $db->bindVars($sql, ':city', $city, 'string');
                $sql = $db->bindVars($sql, ':postcode', $postcode, 'string');
                $sql = $db->bindVars($sql, ':oneWayMiles', $oneWayMiles ?? '', 'string');
                $db->Execute($sql);
                ...
                zen_record_admin_activity(
                    'IEMS unit created with ID ' . $unitId
                        ...
                        . ', one-way miles ' . ($oneWayMiles ?? 'unset'),
                    'info'
                );
            ...
            $sql =
                "UPDATE " . TABLE_IEMS_UNITS . "
                    SET county_ID = :countyId,
                        ...
                        unit_name = :unitName,
                        delivery_street_address = NULLIF(:streetAddress, ''),
                        delivery_city = NULLIF(:city, ''),
                        delivery_postcode = NULLIF(:postcode, ''),
    ...
    if ($unitId === null) {
        ...
        if ($unit === null) {
            ...
            $formValues = [
                ...
                'delivery_street_address' => (string)($unit['delivery_street_address'] ?? ''),
                'delivery_city' => (string)($unit['delivery_city'] ?? ''),
                'delivery_postcode' => (string)($unit['delivery_postcode'] ?? ''),
...
$unitsQueryRaw =
    "SELECT u.unit_ID, u.county_ID, u.agency_ID, u.unit_identifier, u.unit_name,
            u.delivery_street_address, u.delivery_city, u.delivery_postcode, u.one_way_miles, u.status,
...
<div class="container-fluid">
    ...
    <div class="alert alert-info"><?php echo TEXT_ADDRESS_CONSEQUENCE; ?></div>
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                <fieldset>
                    ...
                    <p class="help-block"><?php echo TEXT_DELIVERY_FIXED_REGION; ?></p>
                    <div class="form-group">
                        <label class="control-label col-sm-3" for="delivery_street_address"><?php echo TEXT_DELIVERY_STREET_ADDRESS; ?></label>
                        <div class="col-sm-9">
                            <?php
                            echo zen_draw_input_field(
                                'delivery_street_address',
                                $formValues['delivery_street_address'],
                                'id="delivery_street_address" class="form-control" maxlength="128"'
                            );
                            ?>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-sm-3" for="delivery_city"><?php echo TEXT_DELIVERY_CITY; ?></label>
                        <div class="col-sm-9">
                            <?php
                            echo zen_draw_input_field(
                                'delivery_city',
                                $formValues['delivery_city'],
                                'id="delivery_city" class="form-control" maxlength="128"'
                            );
                            ?>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-sm-3" for="delivery_postcode"><?php echo TEXT_DELIVERY_POSTCODE; ?></label>
                        <div class="col-sm-9">
                            <?php
                            echo zen_draw_input_field(
                                'delivery_postcode',
                                $formValues['delivery_postcode'],
                                'id="delivery_postcode" class="form-control" maxlength="64"'
                            );
                            ?>
                        </div>
                    </div>
                ...
                <div class="form-group">
                    <label class="control-label col-sm-3" for="one_way_miles"><?php echo TEXT_ONE_WAY_MILES; ?></label>
                    <div class="col-sm-9">
                        <?php
                        echo zen_draw_input_field(
                            'one_way_miles',
                            $formValues['one_way_miles'] ?? '',
                            'id="one_way_miles" class="form-control" inputmode="decimal" maxlength="8" pattern="[0-9]{1,5}([.][0-9]{1,2})?"'
                        );
                        ?>
                        <p class="help-block"><?php echo TEXT_ONE_WAY_MILES_HELP; ?></p>
                    </div>
                </div>
    ...
    <div class="table-responsive">
        <table class="table table-striped table-hover">
            ...
            <tr>
                ...
                <th class="text-center"><?php echo TEXT_ADDRESS_READY; ?></th>
                <th><?php echo TEXT_ONE_WAY_MILES; ?></th>
            ...
            <?php if ($units->EOF) { ?>
                <tr>
                    <td colspan="9" class="text-center"><?php echo TEXT_NO_UNITS; ?></td>
                </tr>
            <?php } ?>
            <?php while (!$units->EOF) { ?>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <?php
                        $addressReady = trim((string)$units->fields['delivery_street_address']) !== ''
                            && trim((string)$units->fields['delivery_city']) !== ''
                            && trim((string)$units->fields['delivery_postcode']) !== '';
                        ?>
                        <span class="label <?php echo $addressReady ? 'label-success' : 'label-default'; ?>">
                            <?php echo $addressReady ? TEXT_ADDRESS_READY_YES : TEXT_ADDRESS_READY_NO; ?>
                        </span>
                    </td>
                    <td>
                        <?php
                        $mileage = IemsUnitInput::mileage($units->fields['one_way_miles']);
                        echo $mileage === false
                            ? ERROR_ONE_WAY_MILES_INVALID
                            : zen_output_string_protected($mileage ?? TEXT_ONE_WAY_MILES_UNSET);
                        ?>
                    </td>
                    <td class="text-center">
