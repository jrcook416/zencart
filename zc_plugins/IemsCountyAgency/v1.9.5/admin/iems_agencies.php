...
if (!IemsShippingSchema::isReady($db)) {
    $messageStack->add_session(ERROR_SHIPPING_SCHEMA, 'error');
...
function iems_get_agency(int $agencyId): ?array
{
    ...
    $sql =
        "SELECT agency_ID, county_ID, agency_identifier, agency_name, delivery_enabled, payment_mode, shipping_category, status
...
function iems_add_input_errors(array $errorCodes): void
{
    ...
    $messages = [
        ...
        'delivery_enabled_invalid' => ERROR_DELIVERY_ENABLED_INVALID,
        'payment_mode_invalid' => ERROR_PAYMENT_MODE_INVALID,
        'shipping_category_invalid' => ERROR_SHIPPING_CATEGORY_INVALID,
    ];
...
$formValues = [
    ...
    'agency_name' => '',
    'delivery_enabled' => '0',
    'payment_mode' => IemsAgencyInput::PAYMENT_MODE_INVOICE,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!in_array($postAction, ['save', 'deactivate', 'reactivate'], true)) {
        ...
        $countyNumbers = array_column($counties, 'county_number', 'id');
        if ($existingAgency !== null) {
            $existingAgency['county_number'] = $countyNumbers[(int)$existingAgency['county_ID']] ?? '';
        }
        $category = IemsAgencyInput::shippingCategory(
            $_POST,
            $countyNumbers[$countyId ?? 0] ?? '',
            $existingAgency
        );
        $formValues['shipping_category'] = $category ?? '';
        if ($category === null) {
            $input['errors'][] = 'shipping_category_invalid';
        }
        ...
        if ($input['errors'] === []) {
            ...
            $deliveryEnabled = (int)$formValues['delivery_enabled'];
            $paymentMode = $formValues['payment_mode'];
            $shippingCategory = $formValues['shipping_category'];
            if ($isCreate) {
                $sql =
                    "INSERT INTO " . TABLE_IEMS_AGENCIES . "
                        (county_ID, agency_identifier, agency_name, delivery_enabled, payment_mode, shipping_category,
                         status, date_added, last_modified)
                     VALUES
                        (:countyId, :identifier, :agencyName, :deliveryEnabled, :paymentMode, :shippingCategory,
                         1, NOW(), NULL)";
                $sql = $db->bindVars($sql, ':countyId', $countyId, 'integer');
                $sql = $db->bindVars($sql, ':identifier', $identifier, 'string');
                $sql = $db->bindVars($sql, ':agencyName', $name, 'string');
                $sql = $db->bindVars($sql, ':deliveryEnabled', $deliveryEnabled, 'integer');
                $sql = $db->bindVars($sql, ':paymentMode', $paymentMode, 'string');
                $sql = $db->bindVars($sql, ':shippingCategory', $shippingCategory, 'string');
                $db->Execute($sql);
            ...
            $oldDeliveryEnabled = (int)$existingAgency['delivery_enabled'];
            $oldPaymentMode = (string)$existingAgency['payment_mode'];
            ...
            zen_record_admin_activity(
                'IEMS agency updated for ID ' . $agencyId
                    ...
                    . ', payment mode ' . $oldPaymentMode . ' to ' . $paymentMode
                    . ', shipping category ' . $existingAgency['shipping_category'] . ' to ' . $shippingCategory,
                'info'
            );
        ...
        if ($agencyId === null) {
            ...
            if ($agency === null) {
                $messageStack->add(ERROR_AGENCY_NOT_FOUND, 'error');
            } elseif (IemsAgencyInput::shippingCategory(
                [],
                array_column($counties, 'county_number', 'id')[(int)$agency['county_ID']] ?? '',
                $agency + ['county_number' => array_column($counties, 'county_number', 'id')[(int)$agency['county_ID']] ?? '']
            ) === null) {
                $messageStack->add(ERROR_SHIPPING_CATEGORY_INVALID, 'error');
            } else {
    ...
    if ($agencyId === null) {
        ...
        if ($agency === null) {
            ...
            $formValues = [
                ...
                'delivery_enabled' => (string)(int)$agency['delivery_enabled'],
                'payment_mode' => (string)$agency['payment_mode'],
...
$agenciesQueryRaw =
    "SELECT a.agency_ID, a.county_ID, a.agency_identifier, a.agency_name,
            a.delivery_enabled, a.payment_mode, a.status, a.shipping_category,
    ...
    <?php if ($formMode !== '') { ?>
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                <div class="form-group">
                    <label class="control-label col-sm-3" for="delivery_enabled"><?php echo TEXT_DELIVERY_ENABLED; ?></label>
                    <div class="col-sm-9">
                        <?php
                        echo zen_draw_hidden_field('delivery_enabled', '0');
                        echo zen_draw_checkbox_field(
                            'delivery_enabled',
                            '1',
                            $formValues['delivery_enabled'] === '1',
                            '',
                            'id="delivery_enabled"'
                        );
                        ?>
                        <p class="help-block"><?php echo TEXT_DELIVERY_ENABLED_HELP; ?></p>
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-sm-3" for="shipping_category"><?php echo TEXT_SHIPPING_CATEGORY; ?></label>
                    <div class="col-sm-9">
                        <?php
                        $shippingLabels = [
                            'marion' => TEXT_SHIPPING_CATEGORY_MARION,
                            'iems' => TEXT_SHIPPING_CATEGORY_IEMS,
                            'out_of_county' => TEXT_SHIPPING_CATEGORY_OUT_OF_COUNTY,
                        ];
                        $countyNumbers = array_map(
                            [IemsAgencyInput::class, 'countyNumber'],
                            array_column($counties, 'county_number', 'id')
                        );
                        $selectedCountyNumber = $countyNumbers[(int)$formValues['county_id']] ?? null;
                        $shippingChoices = match ($selectedCountyNumber) {
                            '049' => ['marion', 'iems'],
                            null => [],
                            default => ['out_of_county'],
                        };
                        $shippingOptions = [];
                        if (!in_array($formValues['shipping_category'], $shippingChoices, true)) {
                            $shippingOptions[] = ['id' => '', 'text' => ERROR_SHIPPING_CATEGORY_INVALID];
                        }
                        foreach ($shippingChoices as $choice) {
                            $shippingOptions[] = ['id' => $choice, 'text' => $shippingLabels[$choice]];
                        }
                        echo zen_draw_pull_down_menu(
                            'shipping_category',
                            $shippingOptions,
                            $formValues['shipping_category'],
                            'id="shipping_category" class="form-control" required'
                        );
                        ?>
                        <p class="help-block"><?php echo TEXT_SHIPPING_CATEGORY_HELP; ?></p>
                        <script>
                        (() => {
                            'use strict';
                            const county = document.getElementById('county_id');
                            const category = document.getElementById('shipping_category');
                            const numbers = <?php echo json_encode($countyNumbers, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_THROW_ON_ERROR); ?>;
                            const labels = <?php echo json_encode($shippingLabels, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_THROW_ON_ERROR); ?>;
                            let previousNumber = numbers[county.value];
                            county.addEventListener('change', () => {
                                const number = numbers[county.value];
                                const choices = !number ? [] : (number === '049' ? ['marion', 'iems'] : ['out_of_county']);
                                const selected = number === previousNumber ? category.value : choices[0];
                                category.replaceChildren();
                                choices.forEach((value) => {
                                    category.add(new Option(labels[value], value, false, value === selected));
                                });
                ...
                <div class="form-group">
                    <label class="control-label col-sm-3" for="payment_mode"><?php echo TEXT_PAYMENT_MODE; ?></label>
                    <div class="col-sm-9">
                        <?php
                        echo zen_draw_pull_down_menu(
                            'payment_mode',
                            [
                                [
                                    'id' => IemsAgencyInput::PAYMENT_MODE_INVOICE,
                                    'text' => TEXT_PAYMENT_MODE_INVOICE,
                                ],
                                [
                                    'id' => IemsAgencyInput::PAYMENT_MODE_IEMS_UNIT,
                                    'text' => TEXT_PAYMENT_MODE_IEMS_UNIT,
                                ],
                            ],
                            $formValues['payment_mode'],
                            'id="payment_mode" class="form-control" required'
                        );
                        ?>
                        <p class="help-block"><?php echo TEXT_PAYMENT_MODE_HELP; ?></p>
                    </div>
                </div>
    ...
    <div class="table-responsive">
        <table class="table table-striped table-hover">
            ...
            <tr>
                ...
                <th class="text-center"><?php echo TEXT_DELIVERY_ENABLED; ?></th>
                <th><?php echo TEXT_PAYMENT_MODE; ?></th>
                <th><?php echo TEXT_SHIPPING_CATEGORY; ?></th>
            ...
            <?php if ($agencies->EOF) { ?>
                <tr>
                    <td colspan="10" class="text-center"><?php echo TEXT_NO_AGENCIES; ?></td>
                </tr>
            <?php } ?>
            <?php while (!$agencies->EOF) { ?>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <?php if ((int)$agencies->fields['delivery_enabled'] === 1) { ?>
                            <span class="label label-success"><?php echo TEXT_ENABLED; ?></span>
                        <?php } else { ?>
                            <span class="label label-default"><?php echo TEXT_DISABLED; ?></span>
                        <?php } ?>
                    </td>
                    <td>
                        <?php
                        echo match ($agencies->fields['payment_mode']) {
                            IemsAgencyInput::PAYMENT_MODE_INVOICE => TEXT_PAYMENT_MODE_INVOICE,
                            IemsAgencyInput::PAYMENT_MODE_IEMS_UNIT => TEXT_PAYMENT_MODE_IEMS_UNIT,
                            default => '<span class="label label-danger">' . TEXT_PAYMENT_MODE_INVALID . '</span>',
                        };
                        ?>
                    </td>
                    <td>
                        <?php
                        $storedCategory = IemsAgencyInput::shippingCategory(
                            [],
                            (string)$agencies->fields['county_number'],
                            $agencies->fields
                        );
                        echo match ($storedCategory) {
                            'marion' => TEXT_SHIPPING_CATEGORY_MARION,
                            'iems' => TEXT_SHIPPING_CATEGORY_IEMS,
                            'out_of_county' => TEXT_SHIPPING_CATEGORY_OUT_OF_COUNTY,
                            default => '<span class="label label-danger">' . ERROR_SHIPPING_CATEGORY_INVALID . '</span>',
                        };
                        ?>
                    </td>
