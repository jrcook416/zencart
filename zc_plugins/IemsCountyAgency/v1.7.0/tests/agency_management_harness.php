...
$assert(
    $defaultDelivery['errors'] === [] && $defaultDelivery['values']['delivery_enabled'] === '0',
    'An omitted delivery checkbox should validate as disabled.'
);
...
$assert($manifest['pluginVersion'] === 'v1.7.0', 'Manifest should identify plugin version v1.7.0.');
...
$previousRoot = dirname($pluginRoot) . '/v1.6.0';
$previousFiles = $relativeFiles($previousRoot);
$currentFiles = $relativeFiles($pluginRoot);
$assert(
    array_diff($previousFiles, $currentFiles) === [],
    'v1.7.0 must carry forward the complete v1.6.0 payload.'
);

$intentionallyChangedFiles = [
    'Installer/ScriptedInstaller.php',
    'admin/iems_units.php',
    'admin/includes/classes/IemsUnitInput.php',
    'admin/includes/languages/english/lang.iems_units.php',
    'catalog/includes/classes/IemsCheckoutUnitService.php',
    'catalog/includes/classes/IemsShippingEligibilityService.php',
...
foreach (array_diff($previousFiles, $intentionallyChangedFiles) as $relativePath) {
    ...
    $assert(
        ...
        'v1.6.0 behavior changed unexpectedly: ' . $relativePath
    );
}
