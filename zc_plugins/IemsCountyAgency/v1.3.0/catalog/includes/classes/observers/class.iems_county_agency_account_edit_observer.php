...
class zcObserverIemsCountyAgencyAccountEdit extends base
{
    ...
    public function update(&$class, $eventID, $p1, &$p2, &$p3, &$p4, &$p5, &$p6, &$p7)
    {
        switch ($eventID) {
            ...
            case 'NOTIFY_HEADER_ACCOUNT_EDIT_UPDATES_COMPLETE':
                // Skip persistence when the lock is active — the affiliation has not changed.
                if (!empty($GLOBALS['iems_account_edit_locked'])) {
                    break;
                }
    ...
    private function validateAndRedirectIfInvalid(): void
    {
        ...
        // When the lock observer is active it has already overridden POST with
        // the existing affiliation values, so there is nothing to validate.
        if (!empty($GLOBALS['iems_account_edit_locked'])) {
            return;
        }
