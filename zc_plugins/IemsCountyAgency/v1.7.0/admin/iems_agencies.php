...
function iems_get_agency(int $agencyId): ?array
{
    ...
    $sql =
        "SELECT agency_ID, county_ID, agency_identifier, agency_name, delivery_enabled, status
            ...
            if ($isCreate) {
                $sql =
                    "INSERT INTO " . TABLE_IEMS_AGENCIES . "
                        (county_ID, agency_identifier, agency_name, delivery_enabled, status, date_added, last_modified)
                     VALUES
                        (:countyId, :identifier, :agencyName, :deliveryEnabled, 1, NOW(), NULL)";
                $sql = $db->bindVars($sql, ':countyId', $countyId, 'integer');
                $sql = $db->bindVars($sql, ':identifier', $identifier, 'string');
...
<div class="container-fluid">
    ...
    <div class="table-responsive">
        <table class="table table-striped table-hover">
            ...
            <?php if ($agencies->EOF) { ?>
                <tr>
                    <td colspan="8" class="text-center"><?php echo TEXT_NO_AGENCIES; ?></td>
                </tr>
            <?php } ?>
