...
$assert(
    HEADING_IEMS_CHECKOUT_UNIT === 'Ordering Unit'
        && str_contains($fallbackHtml, '<legend>Ordering Unit</legend>')
        && !str_contains($fallbackHtml, '<legend>IEMS Unit</legend>'),
    'Every checkout selector path must use the exact Ordering Unit heading.'
);

$service->clearSelection();
$_POST = [];
$initialObserver = new zcObserverIemsCheckoutUnit();
$initialHandleShippingPage = new ReflectionMethod($initialObserver, 'handleShippingPage');
$initialHandleShippingPage->invoke($initialObserver);
$assert(
    ($GLOBALS['iems_checkout_initial_unit_selection'] ?? false) === true
        && ($GLOBALS['iems_checkout_shipping_ready'] ?? true) === false,
    'The initial standard shipping page must explicitly identify the unconfirmed selector state.'
);
...
$assert(
    ($GLOBALS['iems_checkout_shipping_ready'] ?? false) === true
        && ($GLOBALS['iems_checkout_initial_unit_selection'] ?? true) === false
...
foreach (array_slice($templateFiles, 0, 3) as $templateFile) {
    ...
    $assert(
        str_contains(
            ...
            && str_contains($contents, 'elseif (!$iems_checkout_initial_unit_selection)')
            && str_contains($contents, 'if (!$iems_checkout_initial_unit_selection)')
            && preg_match(
                '/if\s*\(\s*\$iems_checkout_initial_unit_selection\s*\|\|/',
                $contents
            ) === 1,
        'Initial checkout must hide the no-shipping block and comments, retain continue, and restore normal sections after confirmation: '
            . $templateFile
    );
}
