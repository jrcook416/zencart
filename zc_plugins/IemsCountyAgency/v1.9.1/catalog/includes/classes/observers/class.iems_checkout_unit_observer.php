...
class zcObserverIemsCheckoutUnit extends base
{
    ...
    {
        switch ($eventID) {
            case 'NOTIFY_SHIPPING_MODULE_CALCULATE_CHEAPEST':
                $this->preferPickup($p2, $p3);
                break;
    ...
    private function handleShippingPage(): void
    {
        ...
        $this->prepareSelector();
        $GLOBALS['iems_checkout_initial_unit_selection'] =
    ...
    private function enforceOrderSelection(object $order): void
    {
        ...
        $this->applyOrderAddresses($order, $selection);
        $this->validateDeliverySelection($order);
    }

    private function preferPickup(mixed &$cheapest, array $rates): void
    {
        foreach ($rates as $rate) {
            if (
                ($rate['id'] ?? '') === 'iemspickup_iemspickup'
                && (new IemsShippingEligibilityService())->isPickupEligible($this->customerId())
                && (new IemsShippingAddressService())->isPickupConfigured()
                && defined('MODULE_SHIPPING_IEMSPICKUP_STATUS')
                && MODULE_SHIPPING_IEMSPICKUP_STATUS === 'True'
            ) {
                $cheapest = $rate;
                return;
            }
    ...
    private function validateDeliverySelection(?object $order = null): void
    {
        if ($this->selectedShippingModule() !== IemsShippingAddressService::MODULE_DELIVERY) {
            return;
        }

        $price = (new IemsShippingEligibilityService())->getDeliveryPrice($this->customerId());
        $methodTitle = $price !== null ? IemsShippingEligibilityService::deliveryTitle($price['category']) : null;
        $title = $_SESSION['shipping']['title'] ?? null;
        $cost = $_SESSION['shipping']['cost'] ?? null;
        $valid = $price !== null
            && $methodTitle !== null
            && defined('MODULE_SHIPPING_IEMSDELIVERY_STATUS')
            && MODULE_SHIPPING_IEMSDELIVERY_STATUS === 'True'
            && ($_SESSION['shipping']['id'] ?? '') === 'iemsdelivery_iemsdelivery'
            && is_string($title)
            && str_ends_with($title, ' (' . $methodTitle . ')')
            && (is_int($cost) || is_float($cost) || is_string($cost))
            && is_numeric($cost)
            && (float)$cost === $price['cost'];
        if ($valid && $order !== null) {
            $valid = ($order->info['shipping_method'] ?? null) === $title
                && is_numeric($order->info['shipping_cost'] ?? null)
                && (float)$order->info['shipping_cost'] === $price['cost'];
        }
        if (!$valid) {
            unset($_SESSION['shipping']);
            $this->redirectWithError(
    ...
    private function errorMessage(string $error): string
    {
        return match ($error) {
            ...
            'shipping_changed' => ERROR_IEMS_CHECKOUT_SHIPPING_CHANGED,
