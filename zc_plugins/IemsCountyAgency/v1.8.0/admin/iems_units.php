...
function iems_units_get_unit(int $unitId): ?array
{
    ...
    $sql =
        "SELECT unit_ID, county_ID, agency_ID, unit_identifier, unit_name,
                delivery_street_address, delivery_city, delivery_postcode, status
                    ...
                    "INSERT INTO " . TABLE_IEMS_UNITS . "
                     ...
                     VALUES
                        (:countyId, :agencyId, :identifier, :unitName,
                         NULLIF(:streetAddress, ''), NULLIF(:city, ''), NULLIF(:postcode, ''),
                         1, NOW(), NULL)";
                $sql = $db->bindVars($sql, ':countyId', $countyId, 'integer');
                $sql = $db->bindVars($sql, ':agencyId', $agencyId, 'integer');
                $sql = $db->bindVars($sql, ':identifier', $identifier, 'string');
...
$unitsQueryRaw =
    "SELECT u.unit_ID, u.county_ID, u.agency_ID, u.unit_identifier, u.unit_name,
            u.delivery_street_address, u.delivery_city, u.delivery_postcode, u.status,
...
<div class="container-fluid">
    ...
    <div class="table-responsive">
        <table class="table table-striped table-hover">
            ...
            <?php if ($units->EOF) { ?>
                <tr>
                    <td colspan="8" class="text-center"><?php echo TEXT_NO_UNITS; ?></td>
                </tr>
            <?php } ?>
            <?php while (!$units->EOF) { ?>
