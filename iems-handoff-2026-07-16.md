...
- IemsCountyAgency v1.9.4 reduces the administrative Orders delivery block for recognized IEMS methods to two lines: county code plus agency identifier, then the selected unit or agency fallback. Other order displays and stored addresses remain unchanged.
...
### v1.9.4 deployment checks

- Upgrade through Plugin Manager and open unit-based and agency-fallback orders in **Admin > Customers > Orders**.
- Verify the delivery block shows only `<county code>, <agency identifier>` on line one and the selected unit or agency on line two, with no zone/state or country.
- Verify non-IEMS and malformed historical orders retain standard Zen Cart address formatting; verify invoices and packing slips are unchanged.
- Run all v1.9.4 harnesses, all v1.9.2 regression harnesses, and PHP lint.
