...
class iems_pickup extends ZenShipping
{
    ...
    {
        $this->code = 'iems_pickup';
        if ($uninstalling) {
            return;
        }

        $this->title = MODULE_SHIPPING_IEMS_PICKUP_TEXT_TITLE;
        $this->description = MODULE_SHIPPING_IEMS_PICKUP_TEXT_DESCRIPTION;
        $this->sort_order = defined('MODULE_SHIPPING_IEMS_PICKUP_SORT_ORDER')
            ? (int)MODULE_SHIPPING_IEMS_PICKUP_SORT_ORDER
            : null;
        ...
        $this->enabled = MODULE_SHIPPING_IEMS_PICKUP_STATUS === 'True';
        $this->update_status();
    }
    ...
    public function quote($method = ''): array
    {
        ...
        if ($this->enabled) {
            $methods[] = [
                'id' => $this->code,
                'title' => MODULE_SHIPPING_IEMS_PICKUP_TEXT_WAY,
                'cost' => 0.00,
            ];
        }
        ...
        $this->quotes = [
            'id' => $this->code,
            'module' => MODULE_SHIPPING_IEMS_PICKUP_TEXT_TITLE,
            'methods' => $methods,
        ];
        ...
        if (!isset($this->_check)) {
            $result = $db->Execute(
                "SELECT configuration_value
                  ...
                  WHERE configuration_key = 'MODULE_SHIPPING_IEMS_PICKUP_STATUS'"
            );
