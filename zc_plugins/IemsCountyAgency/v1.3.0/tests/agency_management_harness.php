...
$assert($manifest['pluginVersion'] === 'v1.3.0', 'Manifest should identify plugin version v1.3.0.');
...
$previousRoot = dirname($pluginRoot) . '/v1.2.5';
$previousFiles = $relativeFiles($previousRoot);
$currentFiles = $relativeFiles($pluginRoot);
$assert($previousFiles === $currentFiles, 'v1.3.0 must carry forward the complete v1.2.5 payload.');
...
foreach (array_diff($previousFiles, $intentionallyChangedFiles) as $relativePath) {
    ...
    $assert(
        ...
        'v1.2.5 behavior changed unexpectedly: ' . $relativePath
    );
}
